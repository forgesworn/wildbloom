...
## Installed Safari gate

On macOS, enable Apple's native WebDriver service once and run the installed
Safari journey separately:

```sh
sudo safaridriver --enable
npm run build
npm run acceptance:safari
```

The on-demand `safari-acceptance` workflow runs the same gate on a macOS 26
GitHub runner and records its installed SafariDriver version. It uses the
Safari product's isolated native automation window, not Playwright WebKit. One
session proves a trustworthy production origin, no controlled-service request
on load, exact external-signature encrypted upload, two-event relay
publication, exact self-recovery and independent known-answer recovery,
wrong-key rejection without stale output, one-shot relay timeout,
partial-response cancellation, denied-service failure and page-lifecycle
authority clearing.

SafariDriver permits only one Safari automation session at a time. The
installed-Safari gate therefore does not restate the two-browser peer claim;
the controlled WebRTC swarm and branded-Firefox gates remain the peer-delivery
evidence, and cross-device packet capture remains a release gate.
